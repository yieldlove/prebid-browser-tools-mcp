...
const YL_LARGE_LOGS = Object.freeze({
  BID_REQUESTS: "Bids Requested for Auction with id",
  BIDS_RECEIVED: "Bids Received for Auction with id",
  ADSERVER_REQUEST_SENT: "Sending adserver request for",
})

const YL_LARGE_LOGS_REGEX = new RegExp(
  Object.values(YL_LARGE_LOGS).map(str => str + ".*").join("|")
)
...
async function sendToBrowserConnector(logData) {
  ...
  if (logData.type === "network-request") {
    ...
    serverUrl += logData.endpointPath;

    return sendToServer(logData, serverUrl);
  }
  else if (
  ...
  return new Promise((resolve, reject) => {
    ...
    chrome.debugger.sendCommand(
      ...
      (res) => {
        ...
        const result = res.result?.value; 1
        if (result) {
          for (const [key, value] of Object.entries(YL_LARGE_LOGS)) {
            const regex = new RegExp(value);
            const isMatch = args.find(({ value }) => regex.test(value));
            if (isMatch) {
              entry = {
                type: 'large-log',
                endpointPath: key.toLowerCase().replace("_", "-"),
